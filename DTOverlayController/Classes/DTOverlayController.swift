...
open class DTOverlayController: UIViewController, UIViewControllerTransitioningDelegate {
    ...
    /// Default value is 10.
    public var layOverViewCornerRadius: CGFloat = 10 {
        didSet {
            if viewIfLoaded != nil {
                updateCornerRadius()
            }
    ...
    public var layOverHeight: DTLayOverHeight
    ...
    public init(viewController: UIViewController, layOverHeight: DTLayOverHeight = .static(700), dismissableProgress: CGFloat = 0.4) {
    ...
    private func updateCornerRadius() {
        if #available(iOS 11.0, *) {
            ...
            viewController.view.layer.cornerRadius = layOverViewCornerRadius
        } else {
    ...
    private func addCornerRadiusMask() {
        let path = UIBezierPath(roundedRect: viewController.view.bounds, byRoundingCorners: [.topLeft, .topRight], cornerRadii: CGSize(width: layOverViewCornerRadius, height: layOverViewCornerRadius))
    ...
    open func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source: UIViewController) -> UIPresentationController? {
        return DTOverlayPresentationController(presentedViewController: presented, presenting: presenting, layOverHeight: layOverHeight)
    }
...
extension UIViewController {
    public var layOverController: DTOverlayController? {
        var parentViewController = parent
        while parentViewController != nil {
            if let layOverController = parentViewController as? DTOverlayController {
                return layOverController
            } else {
